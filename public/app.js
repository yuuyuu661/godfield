...
// ---------- Roulette with accurate pointer + delete + audio ----------
const wheel = $("#wheel");
...
$("#spin").addEventListener("click", () => spin(false));
$("#spinRemove").addEventListener("click", () => spin(true));
...
// Convert final rotation -> index at pointer (top = -90deg)
function indexAtPointer(n, rotation) {
  const seg = (Math.PI*2)/n;
  // Angle on the wheel that is under the pointer
...
function spin(removePicked) {
  ...
  const baseTurns = Math.PI*2*(4 + Math.random()*3); // 4-7 turns
  ...
  function animate(now) {
    ...
    if (t < 1) {
      ...
      // Finish
      spinning = false;
      // Compute result by pointer mapping
